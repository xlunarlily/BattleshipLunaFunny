...
void main() {
    ...
    while(!fieldAttR.checkOver() && !fieldDefR.checkOver()){
        if(turnNum%2 == 0){
            takeTurn(fieldDefR, fieldDefS, turnNum);
        }
        else{
...
void setShips(Battlefield field){
    ...
    while(x < 5) {
        ...
        if(orient.equalsIgnoreCase("horizontal")){
            int colStart = -1;
            System.out.println("\nEnter a starting column between 0 and " + (9 - currShip.getLength()));
            ...
            while ((!(rowUse >= 0 && rowUse < 10) || !larp)) {
                ...
                if (rowUse >= 0 && rowUse < 10) {
                    ...
                    if(larp){
                        ...
                        //Get a check somehwere that the entire row isn't taken up, make it say between some
                        //variable and the next open row, maybe also for completely taken up rows in the middle somewhere
                        //to get two ranges, like 1 to 3 and 5 to 9 or something, very rare, but could happen
                        //maybe a while loop with the incramenting variable being the first open row as it checks
                        //if occupied row > 0 && occupied row < 9 have special print statements, this will be in main method
                        //I need a method that returns int of occupied row, or i have it all in one method
                        //I will need it for column as well, put it in first for loop that asks
                        //for the starting col and starting row
                    }
                }
            ...
            int rowStart = -1;
            System.out.println("\nEnter a starting row between 0 and " + (9 - currShip.getLength()));
